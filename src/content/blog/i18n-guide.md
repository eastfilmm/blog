---
title: '웹사이트에 번역 시스템 넣으려면 무엇부터 해야 할까: i18n 구조 설계 가이드'
description: '웹사이트에 i18n을 도입할 때 로케일 결정, 번역 파일 구조, namespace 설계, 성능 최적화까지 실무 순서로 정리한 가이드.'
pubDate: '2026-04-19'
...
tags:
  ...
  - AEO
  - frontend
keywords: 'i18n, next-intl, i18next, react-intl, 번역시스템, 다국어, AEO, 프론트엔드'
faq:
  - question: '웹사이트에 번역 시스템을 넣으려면 무엇부터 해야 할까?'
    answer: '문자열 번역이 아니라 구조 설계부터 시작해야 한다. 지원 언어와 기본 로케일, 메시지 파일 구조, 라우팅 방식, 라이브러리 선택, namespace 분리, lazy loading 전략 순서로 설계한다.'
  - question: 'i18n은 문자열만 번역하면 끝나는 문제일까?'
    answer: '아니다. i18n은 로케일 결정, 번역 리소스 선택·로딩, 메모리 적재, 런타임 조회, 렌더링 타이밍을 모두 포함하는 시스템이다.'
  - question: '로케일은 어떻게 결정해야 할까?'
    answer: 'URL 경로 → 사용자 명시 설정(쿠키/로컬스토리지) → 브라우저 Accept-Language → fallback 순으로 판단한다.'
  - question: 'i18n 라이브러리는 실제로 어떤 역할을 할까?'
    answer: '현재 로케일을 확인하고 번역 JSON을 로딩·파싱해 메모리에 적재한 뒤, t(key) 호출 시 값을 반환하는 번역 런타임이다.'
  - question: '번역 파일은 왜 나눠야 할까?'
    answer: '모든 번역을 하나의 JSON에 담으면 초기 다운로드·파싱·메모리 점유가 커지고 첫 렌더가 지연된다. 페이지·기능 단위로 나눠야 필요한 번역만 필요한 시점에 로딩할 수 있다.'
  - question: 'namespace는 왜 필요한가?'
    answer: 'namespace는 번역 리소스를 논리 단위로 분리하는 스코프이자 로딩 단위다. 번역판 코드 스플리팅 역할을 한다.'
  - question: '번역 시스템의 병목은 어디서 발생할까?'
    answer: 't(key) 조회가 아니라 번역 JSON의 네트워크 요청·다운로드·파싱·메모리 적재 단계에서 주로 발생한다.'
  - question: 'next-intl, i18next, react-intl 중 무엇을 선택해야 할까?'
    answer: 'Next.js App Router는 next-intl, React SPA/Vite는 i18next + react-i18next, 포맷팅 표현력이 중요하면 react-intl이 적합하다.'
  - question: '번역 시스템은 어떤 순서로 구축해야 할까?'
    answer: '1) 지원 언어·기본 로케일 결정 2) 라우팅/상태 방식 결정 3) 메시지 구조 설계 4) 라이브러리 선택 5) 번역 호출 방식 통일 6) lazy loading 전략 7) 운영 규칙 순이다.'
---
